...
def key_down(event):
    ...
    if not flag:
        if event.char.isdigit():
            ...
            erase()
        # Agregue porcentual, exponente y raíz a la calculadora.
        elif event.char == "%":  # Double click
            click("/100")
        elif event.char == "^":  # Double click
            click("**")
        elif event.char == "√":  # If that key existed
            click("**0.5")
        else:
...
# add erase button
button_erase = tk.Button(root, text="DEL", padx=40, pady=26, command=lambda: erase())
button_erase.config(bg="gray", fg="white", font=("Arial", 20, "bold"), width=2, height=2)
button_erase.grid(row=1, column=4)

# Agregue porcentual, exponente y raíz a la calculadora.
button_percent = tk.Button(root, text="%", padx=40, pady=26, command=lambda: click("/100"))
button_percent.config(bg="red", fg="white", font=("Arial", 20, "bold"), width=2, height=2)
button_percent.grid(row=3, column=4)
button_exp = tk.Button(root, text="^", padx=40, pady=26, command=lambda: click("**"))
button_exp.config(bg="red", fg="white", font=("Arial", 20, "bold"), width=2, height=2)
button_exp.grid(row=4, column=4)
button_sqrt = tk.Button(root, text="√", padx=40, pady=26, command=lambda: click("**0.5"))
button_sqrt.config(bg="red", fg="white", font=("Arial", 20, "bold"), width=2, height=2)
